...
from unison_ignoregit.parser import GitIgnoreToUnisonIgnore, LazyCompiledGitWildMatch
...
def test_special_characters():
    parsed = GitIgnoreToUnisonIgnore("/").parse_ignoregit(StringIO("~-*"))
    assert len(parsed) == 1
    assert str(parsed[0]) == r"-ignore=Regex ^(.+/)?~-[^/]*(/.*)?$"


def test_root_path_removes_slash():
    parsed = GitIgnoreToUnisonIgnore("/").parse_ignoregit(StringIO("test.py"))
    assert len(parsed) == 1
    assert parsed[0].anchor_path == ""


def test_hash_should_not_be_escaped():
    parsed = GitIgnoreToUnisonIgnore(".#*").parse_ignoregit(StringIO(".#file"))
    assert len(parsed) == 1
    assert str(parsed[0]) == r"-ignore=Regex ^.#*/(.+/)?\.#file(/.*)?$"


def test_space_should_not_be_escaped():
    parsed = GitIgnoreToUnisonIgnore("/").parse_ignoregit(StringIO("Hello World"))
    assert len(parsed) == 1
    assert str(parsed[0]) == r"-ignore=Regex ^(.+/)?Hello World(/.*)?$"


def test_accepts_array_of_strings():
    parsed = GitIgnoreToUnisonIgnore("/").parse_ignoregit(["test1.py", "test2.py"])
    assert len(parsed) == 2


def test_keeps_trailing_slash_but_removes_leading_slash():
    parsed = GitIgnoreToUnisonIgnore("/a/b/c/").parse_ignoregit(StringIO("test.py"))
    assert len(parsed) == 1
    assert parsed[0].anchor_path == "a/b/c/"


def test_empty_ignoregit(mock_path):
    parsed = GitIgnoreToUnisonIgnore(mock_path).parse_ignoregit(StringIO())
    assert parsed == []
...
def test_blank_lines_and_comments(mock_path):
    ...
    """
    parsed = GitIgnoreToUnisonIgnore(mock_path).parse_ignoregit(StringIO(contents))
    assert parsed == []
